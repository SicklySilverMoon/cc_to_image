...
def load_images(tileset_folder):
    ...
    for x in range(7):
        for y in range(16):
            bases.append(base_img.crop((x * tile_size, y * tile_size, (x+1) * tile_size, (y+1) * tile_size)))
            overlays.append(overlay_img.crop((x * tile_size, y * tile_size, (x+1) * tile_size, (y+1) * tile_size)))
    base_img.close()
    overlay_img.close()
    return (bases, overlays, tile_size)

def draw_map(img_out, level, base_tiles, overlay_tiles, tile_size):
    for cell_num in range(len(level.map)):
        cell = level.map[cell_num]
        if cell.bottom == CC1.FLOOR:
            img_out.paste(im=base_tiles[cell.top.value], box=(cell_num % 32 * tile_size, cell_num // 32 * tile_size))
        else:
            img_out.paste(im=base_tiles[cell.bottom.value], box=(cell_num % 32 * tile_size, cell_num // 32 * tile_size))
            #img_out.alpha_composite(im=overlays[cell.top.value], dest=(cell_num % 32 * tile_size, cell_num // 32 * tile_size))
            img_out.paste(im=overlay_tiles[cell.top.value], box=(cell_num % 32 * tile_size, cell_num // 32 * tile_size), mask=overlay_tiles[cell.top.value])
...
def parse_is_valid_range(str_in):
...
def main():
    ...
    parser.add_argument("-l", "--levels", help="Levels to map, accepts ranges, ex. -l 1 3 5-7", type=parse_is_valid_range, nargs="+")
    ...
    bases, overlays, tile_size = load_images(args.tileset_folder)
    ...
    #for i in range(4,5):
        ...
        draw_map(img_out, level, bases, overlays, tile_size)
        ...
        img_out.save(f"maps/{dat_name}/{i:05d}.png", "PNG")
